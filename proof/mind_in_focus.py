...
import webbrowser  # <-- added for lofi music
...
# IMPORTANT: don't hard-code your key; use env var instead.
...
def start_lofi_music():
    """
    Open a lofi playlist in the default web browser.
    User can pause/stop it by stopping playback in the browser tab.
    """
    try:
        # Popular lofi hip hop radio stream
        url = "https://www.youtube.com/watch?v=jfKfPfyJRdk"
        webbrowser.open(url)
        print("[MUSIC] Opening lofi playlist in your browser...")
    except Exception as e:
...
def get_quick_fact_for_energy_drink(drink_count: int) -> str | None:
    ...
    """
    if not client or OPENAI_API_KEY in (None, "", "YOUR_OPENAI_API_KEY_HERE"):
        return None
...
def extract_text_from_file(path: str) -> str | None:
    ...
    if ext == ".txt":
        try:
            ...
            print(f"[QUIZ] Error reading text file: {e}")
            return None
    ...
    if ext == ".pdf":
        try:
            ...
            print("[QUIZ] PyPDF2 not installed. Run 'pip install PyPDF2' to enable PDF reading.")
            return None
        ...
        try:
            ...
            print(f"[QUIZ] Error reading PDF file: {e}")
            return None

    print(f"[QUIZ] Unsupported file type: {ext}. Use .txt or .pdf.")
    return None


def get_study_material_from_user() -> str | None:
    """
    After a session, ask the user how to provide study content
    (summary text or file). Returns the text content or None if skipped.
    """
    print("\n📘 Post-session check-in")
    print("Would you like a quick quiz based on what you just studied?")
    print("1) Type a short summary of what you studied")
    print("2) Provide a file path (PDF or text notes / practice test)")
    print("3) Skip quiz for now")
    ...
    if choice == "3":
        print("[QUIZ] Skipping post-session quiz.")
        return None

    if choice == "1":
        print("\nType or paste a short summary of what you studied.")
        ...
        summary = "\n".join(lines).strip()
        if not summary:
            print("[QUIZ] No summary entered. Skipping quiz.")
            return None
        return summary

    if choice == "2":
        path = input("Enter the full path to your PDF or text file: ").strip()
        content = extract_text_from_file(path)
        if not content:
            print("[QUIZ] Could not read content from file. Skipping quiz.")
            return None
        return content
...
def generate_quiz_from_material(material_text: str, num_questions: int = 5):
    """
    Use OpenAI to generate multiple-choice questions (A-D).
    Returns a list of question dicts or None on error.

    Each question dict has:
      - question: str
      - options: { "A": str, "B": str, "C": str, "D": str }
      - answer: "A"/"B"/"C"/"D"
      - topic: short string about the subtopic
      - review_hint: 1–2 sentence suggestion for review
    """
    if not client or OPENAI_API_KEY in (None, "", "YOUR_OPENAI_API_KEY_HERE"):
        print("[QUIZ] OPENAI_API_KEY not set. Cannot generate quiz.")
        return None

    # Trim very long material to keep token usage reasonable
    trimmed = material_text
    max_chars = 4000
    if len(trimmed) > max_chars:
        trimmed = trimmed[:max_chars]

    system_prompt = (
        "You are a helpful study assistant. Given some study material, "
        "you create clear, exam-style multiple-choice questions (A–D) "
        "for a student to self-test."
    )

    user_prompt = f"""
From the material below, create {num_questions} multiple-choice questions.

Requirements:
- Each question should focus on one key concept or fact.
- Each question must have exactly four options labeled A, B, C, and D.
- Exactly one option is correct.
- Questions should be appropriate difficulty for a college or advanced high-school student.
- Provide a short 'topic' label (e.g., 'Photosynthesis basics') for each question.
- Provide a 1–2 sentence 'review_hint' telling the student what to re-read or practice if they miss it.
...
\"\"\"{trimmed}\"\"\"    
    ...
    try:
        ...
        if not isinstance(questions, list) or not questions:
            print("[QUIZ] No questions generated or bad format.")
            return None
        # Basic sanity checks
        cleaned = []
        for q in questions:
            if (
                ...
                and "answer" in q
                and "topic" in q
                and "review_hint" in q
            ):
                cleaned.append(q)
        if not cleaned:
            print("[QUIZ] Questions were not in expected format.")
            return None
        return cleaned
    except json.JSONDecodeError:
        print("[QUIZ] Failed to parse quiz JSON from OpenAI response.")
        return None
    except Exception as e:
...
def run_post_session_quiz() -> bool:
    """
    Full flow:
    - Ask user how to provide material (summary/file/skip)
    - Generate quiz with OpenAI
    - Ask questions in terminal
    - Suggest topics to review based on incorrect answers
    - If score is low, ask if they want to resume a new study session.
    Returns True if user wants to resume, else False.
    """
    material = get_study_material_from_user()
    if not material:
        return False

    questions = generate_quiz_from_material(material)
    if not questions:
        return False

    print("\n📝 Starting quick review quiz!")
    print("Type A, B, C, or D for each question. Press Enter after your choice.\n")
    ...
    for i, q in enumerate(questions, start=1):
        ...
        if ans == correct:
            ...
            print(f"   {q.get('review_hint', '').strip()}\n")
            wrong_topics.append(
                {
                    "topic": q.get("topic", "Unlabeled topic"),
                    "review_hint": q.get("review_hint", "").strip(),
                }
            )
    ...
    score = correct_count / total_q

    if not wrong_topics:
        print("\n🎉 Great job! You answered all questions correctly.")
        print("Do a quick skim of your notes just to keep everything fresh.\n")
        return False
    ...
    print("\n📚 Review suggestions based on what you missed:")
    for item in unique_hints:
        print(f"- {item['topic']}: {item['review_hint']}")
    print()

    # If they "failed" (e.g., <70%), offer to resume previous session
    if score < 0.7:
        print(f"Your quiz score was {correct_count}/{total_q} "
              f"({score*100:.0f}%).")
        choice = input("Do you want to start another focused study session on this same topic now? (y/n): ").strip().lower()
        if choice == "y":
            return True
...
def run_study_session(user: dict, session_meta: dict):
    ...
    play_music = session_meta.get("play_music", False)
    ...
    print(f"Background music:{'Yes' if session_meta.get('play_music', False) else 'No'}")
    if last_quick_fact:
    ...
    # -------------------------
    # Optional post-session quiz
    # -------------------------
    resume = run_post_session_quiz()
    if resume:
        print("\n🔁 Starting another study session on the same topic based on your quiz results.\n")
        run_study_session(user, session_meta)
...
def chatbot_intake(user_email: str):
    ...
    play_music = False
    music_ans = input("Would you like relaxing lofi music during this session? (y/n): ").strip().lower()
    if music_ans == "y":
        play_music = True
    ...
    print(f"- Background music:{'Yes' if play_music else 'No'}")
